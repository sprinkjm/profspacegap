...
// C/C++ source code generated on : Thu Oct 17 10:51:48 2024
//
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"profspacegap\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(profspacegap_M, (NULL));
        profspacegap_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(profspacegap_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(profspacegap_M)
                == (NULL)) || rtmGetStopRequested(profspacegap_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(profspacegap_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
